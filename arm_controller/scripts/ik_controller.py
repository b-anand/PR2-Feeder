...
def control_arm_joints():
    ...
    global robot_state
    positions = robot_state.right_arm_positions
    """
    move_right_arm(positions)
    """
    step = 0.05
...
def get_extrapolated_location(a, b, d):
    a = np.array(a)
    b = np.array(b)
    t = a - b
...
def get_spoon_lowest_position(positions_list=None, spoon_length=0.1):
    ...
    print b
    return get_extrapolated_location(a, b, spoon_length)
...
def move_torso_position(position):
    ...
    sz = 0.801 # start state hieght of torso_lift_link
...
def addPositionConstraint(goal, x, y, z, frame="odom_combined"):
...
def imped_arm_control(x, y, z, arm='right_arm', frame='odom_combined'):
    control = EECartImpedClient(arm)
    
    control.addTrajectoryPoint(x, y, z, 0.7, 0, -0.7, 0.447,
